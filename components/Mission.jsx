        ...
        <table className='table-auto overflow-x-auto'>
            ...
            {data && data.map((item)=>{
                return <tr className="even:bg-[#DFDFDF] odd:bg-white" key={item.mission_id}>
                    ...
                    {status?(<td className='p-5 border '> <button
                    className='text-white text-sm bg-red-600 px-3 py-2 rounded-md'>NOT A MEMBER</button></td>)
                    :(<td className='p-5 border'><button
                     className=' text-white text-sm  bg-green-600 px-3 py-2 rounded-md '>ACTIVE MEMBER</button></td>)}
